...
if __name__ == "__main__":
    ...
    new_input_ids = torch.cat(
        [input_ids[:, 0:-1], torch.zeros(1, prompt_num).long(), torch.unsqueeze(input_ids[:, -1], 0)], dim=1)
    new_input_ids2 = torch.cat(
        [input_ids[:, 0:-1], torch.ones(1, prompt_num, ).long(), torch.unsqueeze(input_ids[:, -1], 0)], dim=1)
    print(new_input_ids)
